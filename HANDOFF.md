...
### ✅ 完了：Σ-5 開発ゲーム体感化（2026-04-23 merge）

Σ-4 完了後に判明した「terraform 置いても地形変わらない / feature が円盤 / 狼から
逃げない」体感問題を一括解消。労働 AI（terraform 自発移動 70%）、feature 3D 化
（家・池・畑・鳥居・窯など建物として識別可能）、HUD 進捗バー、狼 flee + 新死因
`fled_to_exhaustion` を導入。死因 47→48 種、wolf_bite : fled_to_exhaustion = 5:1 で
バランス済。詳細は CLAUDE.md「完了フェーズ → Σ-5」参照。
...
| **Ω-12 潮汐** | Σ-3 島地形向け、海面周期 ±、取り残され溺死 | 1 週 | 島難度の体験を厚くする |
