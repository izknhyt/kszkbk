...
### 🚧 最優先：Σ-5 開発ゲーム体感化（1-2 週）

Σ-4 完了後の実プレイで「terraform 置いても地形変わらない」「水路建ててもただの円」
「狼から逃げない」という 3 大体感問題が判明。実装は存在するが **労働 AI が偶発 +
視覚が貧弱 + HUD フィードバック欠落** の三重苦で機能が埋没してる。これを一括解消する。

**4 サブコミット想定**：
- **Σ-5-a 労働 AI**（2-3 日）：`pickWorkTarget(c)` で trait 重み付き自発ターゲット選択。
  30% サボり率でくそざこ味キープ
- **Σ-5-b feature 視覚化**（2-3 日）：water=池、channel=溝+流れアニメ、farm=作物ビルボード成長、
  house/sawmill/shrine/kiln/loom/well/firewatch/generator を既存建物と同じ BoxGeometry 系で建築感演出
- **Σ-5-c HUD フィードバック**（1-2 日）：terraform 進捗バー HTML overlay、
  saturated 光沢、farm Lv 表示、建設時の連鎖ヒント toast
- **Σ-5-d 狼 flee**（1 日）：80px 検知で `state='scared'`、疲労 +8/sec で
  `fled_to_exhaustion` 新死因、sanctuary（家 / 火の見やぐら 40px 内）で庇護

詳細な実装プロンプトは別途起こす（新フェーズを投げる時は §2 テンプレートを使う）。

### Σ-5 完走後の候補（優先度順）

| 候補 | 内容 | 規模 |
|---|---|---|
| **Σ-6 歩行アニメ** | 既存 9 ポーズから walk_a/b/c atlas、0.3 秒切替。Σ-5 で労働が目に見えるようになってから着手するとインパクト倍 | 1 週 |
| **Ω-12 神罰** | 隕石 / 雷撃でストレス発散。3D の煙・爆発 FX 流用 | 1 週 |
| **Ω-12 潮汐** | Σ-3 島地形向け、海面周期 ±、取り残され溺死 | 1 週 |
| **② 病気＆集団感染** | 咳→伝染→パンデミック、温泉 / 薬草で治療 | 2 週 |
| **⑥ カルト宗教化** | 神社拡張、教祖くそざこ、儀式死、フラナ機嫌暴落 | 2 週 |
| **Ω-4 火災拡張** | 消防署、火の広がり、煙パーティクル | 1 週 |

### 優先度：通常

- **Ω-5 拡張**：クマ / 地震 / 野盗
- **Ω-8 指示系統**：ゾーン矩形 / 投げ縄 / 直接命令（Σ-5 の労働 AI が土台として必要）
