...
### ✅ 完了：Σ-5 〜 Σ-5-f 開発ゲーム体感化（2026-04-23 merge）

Σ-4 完了後の実プレイで判明した体感問題を 5 段階で解消：
...
- **建設キャンセル**：右クリックで「❌ 解体」ボタン、80% 返金（後で Σ-5-f で 80% に底上げ）
- **Σ-5-f**：「terraform 見えない / 資源足りない」→ 3D 視認強化（明色 + 1Hz パルス + 白枠）+ 完了 toast + obstacle yields +50% + standard 初期資源底上げ + 切り土報酬 +28%

詳細は CLAUDE.md「完了フェーズ → Σ-5 〜 Σ-5-f」参照。
