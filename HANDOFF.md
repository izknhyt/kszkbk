...
### ✅ 完了：Σ-5 〜 Σ-6-UI 開発ゲーム体感化 + UI overhaul（2026-04-23 〜 24 merge）

Σ-4 完了後の実プレイで判明した体感問題を 7 段階で解消：
- **Σ-5**：「terraform が動かない / feature が円盤 / 狼から逃げない」→ 労働 AI + 3D 化 + 狼 flee
- **Σ-5-e**：「建物が何か分からない / 即完成で工事感ゼロ / 水源建てられない」→ 識別モーダル + 建設工事段階 + 水源建設 + 水流アニメ
- **Σ-5-e-e**：「秒数で確実に建つ → くそざこ作業チーム感がない」→ 数値 pt ベース + 複数人ボーナス（4人で 3x）+ 事故ペナルティ（死亡 -15 / 喧嘩 -5 / 離脱 -2）+ 77 種くそざこセリフ
- **建設キャンセル**：右クリックで「❌ 解体」ボタン、80% 返金
- **Σ-5-f**：「terraform 見えない / 資源足りない（短期）」→ 3D 視認強化（明色 + 1Hz パルス + 白枠）+ 完了 toast + obstacle yields +50% + standard 初期資源底上げ + 切り土報酬 +28%
- **Σ-5-g**：「資源足りない（長期）」→ 茂み自然再生（朝に bush spawn、難度別 1-2 本/朝、上限 = 初期 ×1.5）。岩は固定で計画性デザイン維持
- **Σ-6-UI**：「建設パネル使いにくい / 情報密度が低い」→ 7 カテゴリタブ + リッチツールチップ + 警告パネル + 資源 trend 矢印 + 3 日天気予報 + B/T/Esc/1-7 ショートカット
...
- 2026-04-23：Σ-5-e-e 完了・本流 merge・キリル文字混じりコメント修正。§6 完了サマリを 3 段階一括表記に更新
