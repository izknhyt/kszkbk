...
> 始まる。フラナを発注する時は、コピペ後に **`Chibiwafu` を `Furana` に
> 全置換**してから使う。その他の記述（arm / leg / ear 等）は共通なのでそのまま。
...
- `14_cheeky_hips` — フラナはドヤ顔しない（キャラ違い、凛とした監督者）
- `19_knocked` — フラナは頻繁にボコられない（オプション）
- `15_heavy_cry` — フラナは号泣しない（静かに目を伏せるのみ）
- `16_cower` — フラナは怯え縮こまらない（監督者なので）
