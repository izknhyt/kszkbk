...
## M2.1 Visual Priorities

Σ-8 実装後の見た目確認で、次を優先改善対象にする。

| 優先 | 対象 | 方針 |
|---|---|---|
| 1 | 遠景の海 / 画面下破綻 | 全面 sea plane ではなく `isSea` タイル単位の水面へ寄せる |
| 2 | 段差スケール | 1 段がちびわふより大きすぎるため `ELEV_SCALE` / `ELEV_STEP` を再検証 |
| 3 | カメラ | 自由回転ではなく、低め / 標準 / 真上寄りの角度プリセット |
| 4 | 建物表現 | 旧 points 建物を廃止し、map 上の建物を見た目でも主役化 |
| 5 | props 密度 | Ground props 10 種で地形の素の板感を減らす |

キャラのドット絵化はこの仕様書ではまだ採用しない。別途フル画面モックで、
既存水彩ビルボード版と低解像度ドット/ピクセル版を比較してから判断する。
...
- color space: sRGB
- 読み込み: `stage3d.ts` の `THREE.TextureLoader` で `LinearFilter / ClampToEdgeWrapping / generateMipmaps=false`
- 適用先: `terrainMat.map` / `cliffWallIM.material.map` / water IM 各種
...
overlay を切り出す場合は Row 3 cell を再活用する。
