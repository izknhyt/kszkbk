...
- chunk / LOD
- Unity 移行
- 自由カメラ回転（Y 軸ビルボード前提を壊すため）
- キャラ画像の勝手なドット絵差し替え

## M2.1 設計リセット

この仕様書は地形/通行/描画の目標仕様を定義する。
M2.1 以降のゲーム全体方針は [M2-DIRECTION-RESET.md](M2-DIRECTION-RESET.md) を併せて参照する。

追加方針:

- `points` は建設費用に使わず、スコア/図鑑/村ランク用途へ限定する
- 旧 `BUILDINGS` は map 上の feature/building + 資源 + 労働へ統合する
- スズ / ココン / ルーは廃止予定。新規実装で依存を増やさない
- くそざこ音頭は廃止予定。新規イベントは画面上の建物/天気/食料/地形に因果を持たせる
- フラナはちびわふより 1 段差に強い移動者にする。ただし 2 段差以上や水没は制限する
- カメラは角度プリセットを追加する。自由回転は導入しない
- 最遠景の海表示破綻と高さスケール過大は M2.1 の見た目改善対象
...
現実装は `ELEV_STEP=25`、render 側 `ELEV_SCALE=6`。
1 段差が Three.js 上 150px になり、64px 前後のちびわふに対して大きすぎる。

検証候補:

| 案 | 内容 | 注意 |
|---|---|---|
| A | `ELEV_STEP=25` 維持、`ELEV_SCALE=3-4` | save 互換への影響が小さい |
| B | `ELEV_STEP=8-12` へ細分化、`ELEV_SCALE=4` 前後 | migration、generator、A*、崖閾値の再調整が必要 |

商業見た目重視なら B が有力。ただし先に小さな検証 branch で見た目と pathfinding を確認する。
...
| 同高度 | 可 | 可 |
| 1 段差 + ramp 接続 | 可 | 可 |
| 1 段差 + ramp なし | 原則不可 | 可、cost 高め |
| 2 段差以上 | 不可 | 不可 |
| waterLevel >= 0.35 | 原則不可 | 原則不可 |

フラナが万能移動すると地形開発の意味が壊れるため、例外は 1 段差までに限定する。
...
現実装は全面 sea plane と sand 海底の組み合わせで、最遠景時に画面下が縞状に破綻する。
M2.1 では次を検討する。

- `isSea` タイルだけ水面 InstancedMesh を置く
- 海タイルの地形面を描かない、または暗い海底専用 material にする
- カメラ最遠景でワールド外が見えないよう viewport-aware clamp を入れる
- `CAMERA_MIN_SCALE` を暫定的に上げる

### Camera Presets（M2.1）

自由回転は禁止。代わりに pitch/height/lookAt のプリセットを持つ。
