...
| 後処理 | water コストに soil 10 追加（spec 通り）+ docs 更新 | e440716 |
...
#### ✅ Σ-5-e-e 数値建設化 + 事故ペナルティ + セリフ拡充（2026-04-23 に本流 merge）

| Phase | 内容 | Commit |
|---|---|---|
| Σ-5-e-e-a | 数値 pt ベース建設（CONSTRUCTION_PTS 25/45/80/120）+ 複数人ボーナス（4 人で 3x）+ 事故ペナルティ（死亡 -15、喧嘩 -5、離脱 -2、ランダムサボり）+ 77 種くそざこセリフ | a0881e0 |
| Σ-5-e-e-b | HUD 表示を「🔨 畑 45/100 pt (3人 → 1.8x)」形式に変更 | 679c164 |
| merge | sigma-5-e-e-main → idle-village 統合 | 5be4441 |
| 後処理 | キリル文字混じりコメント修正 + docs 更新 | af1ed4f |

**追加されたセリフプール（chats.ts、77 種）**：
- 完成 12 種、死亡反応 12 + 名前入り 3、喧嘩 12、ココン強気 6、離脱 10、サボり 12、フラナ反応 4、スズ反応 4 + 名前入り 1、ルー反応 3
- 全て 〜わふ 語尾、ちびわふ全員フラナを「ママ」呼び（「フラナママ」厳禁）、動物擬音禁止
- ココンだけ強気・冷淡（「へへっ、ざまあ！」「もっと泣けよ〜」等）

#### ✅ 建設キャンセル機能（2026-04-23 に本流 merge）

| 内容 | Commit |
|---|---|
| 右クリックモーダルに「❌ 解体（半額返金）」ボタン、devLevel<2 のみ。8 種くそざこ嘆きセリフ | 3f0eac5 |

#### ✅ Σ-5-f terraform 視認 + 資源バランス調整 + 解体返金 80%（2026-04-23 に本流 merge）
...
- standard 初期資源：wood 15→20、stone 10→15、plank 2→4、food 20→25
...
#### ✅ Σ-6-UI HUD / 建設パネル overhaul（2026-04-24 に本流 merge）

| Phase | 内容 | Commit |
|---|---|---|
| Σ-6-UI-a | 建設パネル再構成：7 カテゴリタブ（水利/食料/住居/防災/工業/電力/信仰）+ ビルドカード（絵文字+名称+コスト+効果）+ 資源不足 disabled 状態 | 4c433ed |
| Σ-6-UI-b | ホバー 420ms 遅延のリッチツールチップ（コスト/建設時間/要件/効果/備考）+ build-list セクションヘッダー | 569978b |
| Σ-6-UI-c | 警告パネル（食料/狼/建設遅滞、8 秒フェード）+ 資源バー ▲▼→ trend + 危機時赤パルス + 3 日天気予報 HUD | 7e66d86 |
| Σ-6-UI-d | レスポンシブ調整 + ミニマップ z-index + B/T/Esc/1-7 キーボードショートカット + camera-help 拡充 | e36202f |
| merge | modernize-ui-hud-UlsX8 → idle-village 統合 | （本 merge） |
| 後処理 | docs 更新 | （本コミット） |
...
+ terraform 視認強化 + 資源バランス底上げ + 茂み自然再生 + UI overhaul（カテゴリタブ /
ツールチップ / 警告パネル / 資源 trend / 天気予報 / キー操作）が完成。
次は Σ-6 歩行アニメ or Σ-6 水動力 or Ω-8 指示系統 or Ω-12 神罰など（HANDOFF.md §6 参照）。
